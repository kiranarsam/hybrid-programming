...
void dine(int phil_no, int option)
{
...
int main()
{
  // CASE1
	...
	std::vector<std::thread> philosA;

	for (int i = 0; i < nphilosophers; ++i) {
		philosA.push_back(std::move(std::thread{dine, i, 1}));
	}

	for (auto& philo: philosA)
		philo.join();
  ...
  // CASE2
  // reset for option 1, to avoid deadlock
  for (int i = 0; i < nphilosophers; ++i) {
		mouthfuls[i] = 0;
	}

  // Start a separate thread for each philosopher
	std::vector<std::thread> philosB;

	for (int i = 0; i < nphilosophers; ++i) {
		philosB.push_back(std::move(std::thread{dine, i, 0}));
	}

	for (auto& philo: philosB)
		philo.join();
