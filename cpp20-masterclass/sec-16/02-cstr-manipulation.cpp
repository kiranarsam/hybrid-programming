...
int main() {
    ...
    std::cout << "std::strncmp (" << string_data11 << "," << string_data22 << "," << n << ") : " << std::strncmp(string_data11, string_data22, n) << std::endl;
    ...
    while ((result1 = std::strchr(str1, target1)) != nullptr) {
        std::cout << "Found '" << target1 << "' starting at '" << result1 << "'\n";

        // Increment result, otherwise we'll find target at the same location
        ++str1;  // Making the pointer point to the next character
        ++iterations1;
    }
    std::cout << "iterations : " << iterations1 << std::endl;
